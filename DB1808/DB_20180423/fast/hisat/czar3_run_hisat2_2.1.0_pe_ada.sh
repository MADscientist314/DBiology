...
pe_1='/scratch/user/jochum00/DBiology/DB1808_20180423.data/fast/czar/czar3concatR1.fastq'
pe_2='/scratch/user/jochum00/DBiology/DB1808_20180423.data/fast/czar/czar3concatR2.fastq'
...
platform='ILLUMINA'
sample='czar3'
output_bam="/scratch/user/jochum00/DBiology/DB1808_20180423.data/fast/czar/${sample}_pe_aln.bam"

################################################################################
# If you will be using cufflinks downstream, run hisat2 with the --dta-cufflinks option instead of --dta
hisat2 --dta -p $threads --rg-id "$id" --rg "LB:$library" --rg "SM:$sample" --rg "PL:$platform" -x $genome_index_prefix -q -1 $pe_1 -2 $pe_2 -S czar3.sam

samtools view -bS czar3.sam | samtools sort -m 2G -@ $threads - -T $sample -o $output_bam

rm czar3.sam
annotation_file='/scratch/group/digibio/Canis-lupus-familiaris/Canis_familiaris.CanFam3.1.92.gtf'
bam_file="/scratch/user/jochum00/DBiology/DB1808_20180423.data/fast/czar/${sample}_pe_aln.bam"
